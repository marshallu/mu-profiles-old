...
function mu_employee( $atts, $content = null ) {
	...
	if ( $the_query->have_posts() ) {
		...
		if ( $dept_listing && 'row' === $dept_listing ) {
		...
		} elseif ( $dept_listing && 'enhanced' === $dept_listing ) {
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
		...
		} elseif ( $dept_listing && 'full-profile' === $dept_listing ) {
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
		...
		} elseif ( $dept_listing && 'table' === $dept_listing ) {
			...
			if ( 'enhanced' === get_field( 'profile_listing_display', 'option' ) ) {
				while ( $the_query->have_posts() ) {
					$the_query->the_post();
